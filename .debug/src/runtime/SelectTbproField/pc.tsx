...
const FormField: ISwapFormField = {
  ...
  asyncSetFieldProps(vlauedata) {
    ...
    const SelectTbproField = form.getFieldInstance('SelectTbpro');

    // const leaveReasonField = form.getFieldInstance('leaveReason');
    const key = SelectTbproField.getProp('id');
    ...
    spi
      .refreshData({
        modifiedBizAlias: ['SelectTbpro'], // spi接口要改动的是leaveReason的属性值
        bizAsyncData,
      })
      .then(res => {
  ...
  fieldRender() {
    ...
    const label = form.getFieldProp('SelectTbpro', 'label');
    const required = form.getFieldProp('SelectTbpro', 'required');
    const placeholder = form.getFieldProp('SelectTbpro', 'placeholder');
    ...
    return (
      <div className="SelectTbproField_class">
        {' '}
        <div className="pc-custom-field-wrap">
          <div className="label">
            {required ? (
