...
const FormField: ISwapFormField = {
  ...
  asyncSetFieldProps(vlauedata) {
    ...
    const CorpSupplierField = form.getFieldInstance('CorpSupplier');

    // const leaveReasonField = form.getFieldInstance('leaveReason');
    const key = CorpSupplierField.getProp('id');
    ...
    spi
      .refreshData({
        modifiedBizAlias: ['CorpSupplier'], // spi接口要改动的是leaveReason的属性值
        bizAsyncData,
      })
      .then(res => {
  ...
  fieldRender() {
    ...
    const placeholder = form.getFieldProp('CorpSupplier', 'placeholder');
