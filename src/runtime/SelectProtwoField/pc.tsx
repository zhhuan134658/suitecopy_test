...
const FormField: ISwapFormField = {
  ...
  asyncSetFieldProps(vlauedata) {
    ...
    const SelectProtwoField = form.getFieldInstance('SelectProtwo');

    // const leaveReasonField = form.getFieldInstance('leaveReason');
    const key = SelectProtwoField.getProp('id');
    ...
    spi
      .refreshData({
        modifiedBizAlias: ['SelectProtwo'], // spi接口要改动的是leaveReason的属性值
        bizAsyncData,
      })
      .then(res => {
  ...
  fieldRender() {
    ...
    const label = form.getFieldProp('SelectProtwo', 'label');
    const required = form.getFieldProp('SelectProtwo', 'required');
    const placeholder = form.getFieldProp('SelectProtwo', 'placeholder');
    ...
    return (
      <div className="SelectProtwoField_class">
        {' '}
        <div className="pc-custom-field-wrap">
          <div className="label">
            {required ? (
