...
const FormField: ISwapFormField = {
  ...
  asyncSetFieldProps(vlauedata) {
    ...
    const CorpSupplieryiField = form.getFieldInstance('CorpSupplieryi');

    // const leaveReasonField = form.getFieldInstance('leaveReason');
    const key = CorpSupplieryiField.getProp('id');
    ...
    spi
      .refreshData({
        modifiedBizAlias: ['CorpSupplieryi'], // spi接口要改动的是leaveReason的属性值
        bizAsyncData,
      })
      .then(res => {
  ...
  fieldRender() {
    ...
    const placeholder = form.getFieldProp('CorpSupplieryi', 'placeholder');
    ...
    return (
      <div className="CorpSupplieryi_class">
        {' '}
        <div className="pc-custom-field-wrap">
          <div className="label">
            {required ? (
